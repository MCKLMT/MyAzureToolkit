<#
    ...
    .DESCRIPTION
        ...
        Tha Azure Automation RunAs account has to be set up and should have permission to start and stop VMs.
      
    .PARAMETER Simulate
        If $true, the runbook will not perform any power actions and will only simulate evaluating the tagged schedules. Use this
        to test your runbook to see what it will do when run normally (Simulate = $true).

    .EXAMPLE
        You should set the tag "AutoShutdownSchedule" with the schedule on the VM or on the VM's resource group. See below examples:
...
try {
    ...
    # Connection to Azure using the RunAs account of Azure Automation
    $connection = Get-AutomationConnection -Name AzureRunAsConnection
    Connect-AzureRmAccount -ServicePrincipal -Tenant $connection.TenantID -ApplicationID $connection.ApplicationID `
        -CertificateThumbprint $connection.CertificateThumbprint | Out-Null

    # Connection to current subscription
    Select-AzureRmSubscription -SubscriptionId $connection.SubscriptionID | Out-Null                   

    # Get a list of all ARM virtual machines in subscription with the tag "AutoShutdownSchedule"
    $resourceManagerVMList = Get-AzureRmResource -ResourceType "Microsoft.Compute/virtualMachines" | Sort-Object Name

    # Get resource groups that are tagged for automatic shutdown of resources
    $taggedResourceGroups = Get-AzureRmResourceGroup -Tag @{ "AutoShutdownSchedule" = $null } | Sort-Object ResourceGroupName
    ...
    foreach ($vm in $resourceManagerVMList) {
        ...
        if ($vm.ResourceType -eq "Microsoft.Compute/virtualMachines" -and $null -ne $vm.Tags -and $vm.Tags.ContainsKey("AutoShutdownSchedule")) {
            # VM has direct tag (possible for resource manager deployment model VMs). Prefer this tag schedule.
            $schedule = $vm.Tags.Item("AutoShutdownSchedule")
        ...
        elseif ($taggedResourceGroupNames -contains $vm.ResourceGroupName) {
            ...
            $schedule = $parentGroup.Tags.Item("AutoShutdownSchedule")
            Write-Output "[$($vm.Name)]: Found parent resource group [$($vm.ResourceGroupName)] schedule tag with value: $schedule."
        }
        else {
